...
// 格式化章节导出内容
function formatChapter(chapter: Chapter, includeSummary = true, convertToPlainText = true): string {
    const parts: string[] = []
...
// 格式化卷导出内容（包含所有章节）
function formatVolume(volume: Volume, includeSummary = true, convertToPlainText = true): string {
    const parts: string[] = []
...
export function registerFileHandlers(): void {
    ...
    ipcMain.handle(
        ...
        async (_event, filePath: string): Promise<FileResult> => {
            try {
    ...
    ipcMain.handle(
        ...
        async (_event, filePath: string, data: string): Promise<FileResult> => {
            try {
    ...
    ipcMain.handle(
        ...
        async (event, defaultPath: string, type: 'file' | 'dir'): Promise<FileResult> => {
            try {
    ...
    ipcMain.handle(
        ...
        async (_event, config: ExportConfig): Promise<FileResult> => {
            try {
                ...
                if (type === 'all') {
                    ...
                    data.characters.forEach((character) => {
                        exportPromises.push(
                            writeFileAsync(
                                join(baseDir, '人物', `${character.name}.txt`),
                    ...
                    data.worldViews.forEach((worldView) => {
                        exportPromises.push(
                            writeFileAsync(
                                join(baseDir, '设定', `${worldView.name}.txt`),
                    ...
                    data.volumes.forEach((volume) => {
                        ...
                        exportPromises.push(
                            writeFileAsync(
                                volumePath,
                                formatVolume(volume, includeSummary, convertToPlainText)
                            )
                        )
                    })
                }
                ...
                if (type === 'volume' && volumeIndex !== undefined) {
                    ...
                    data.characters.forEach((character) => {
                        exportPromises.push(
                            writeFileAsync(
                                join(volumeDir, '人物', `${character.name}.txt`),
                                formatCharacter(character)
                            )
                        )
                    })
                    ...
                    data.worldViews.forEach((worldView) => {
                        exportPromises.push(
                            writeFileAsync(
                                join(volumeDir, '设定', `${worldView.name}.txt`),
                                formatWorldView(worldView)
                            )
                        )
                    })
                    ...
                    exportPromises.push(
                        writeFileAsync(
                            join(volumeDir, `${volume.volumeName}.txt`),
                            formatVolume(volume, includeSummary, convertToPlainText)
                        )
                    )
                }
                ...
                if (type === 'chapter' && volumeIndex !== undefined && chapterIndex !== undefined) {
                    ...
                    exportPromises.push(
                        writeFileAsync(
                            chapterPath,
                            formatChapter(chapter, includeSummary, convertToPlainText)
                        )
                    )
                }
