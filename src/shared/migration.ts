...
export function migrateData(data: OldNovelData): NovelData {
    ...
    const migrated: NovelData = {
        ...
        agent: migrateAgent(data as any)
    }
...
// 迁移 / 补齐 Agent 数据（v3 及更早无此字段）
function migrateAgent(data: any): AgentBookData {
    const empty: AgentBookData = { sessions: [], activeSessionId: null }
    if (!data || !data.agent || typeof data.agent !== 'object') return empty

    const raw = data.agent
    const sessions: AgentSession[] = Array.isArray(raw.sessions)
        ? raw.sessions.map(migrateAgentSession).filter((s): s is AgentSession => s !== null)
        : []

    const activeId =
        typeof raw.activeSessionId === 'string' &&
        sessions.some((s) => s.id === raw.activeSessionId)
            ? raw.activeSessionId
            : null
...
const VALID_SCOPES: AgentContextScope[] = ['currentChapter', 'outline', 'fullBook']

function migrateAgentSession(raw: any): AgentSession | null {
    if (!raw || typeof raw !== 'object' || typeof raw.id !== 'string') return null

    const now = Date.now()
    const scope: AgentContextScope = VALID_SCOPES.includes(raw.contextScope)
        ? raw.contextScope
        : 'outline'

    const messages: AgentMessage[] = Array.isArray(raw.messages)
        ? raw.messages
    ...
    return {
        id: raw.id,
        title: typeof raw.title === 'string' && raw.title ? raw.title : '未命名会话',
...
export function validateMigratedData(data: NovelData): { valid: boolean; errors: string[] } {
    ...
    if (data.agent !== undefined) {
        if (!data.agent || typeof data.agent !== 'object' || !Array.isArray(data.agent.sessions)) {
            errors.push('助手数据无效')
        }
    }
...
export function parseAndMigrate(fileContent: string): {
    ...
    try {
        ...
        // 无需整本迁移：缺 agent 字段则内存规范化（不触发写回）
        const data = rawData as NovelData
        if (!data.agent || !Array.isArray(data.agent.sessions)) {
            data.agent = { sessions: [], activeSessionId: null }
        }
        return { success: true, data, migrated: false }
    } catch (error) {
        return {
            success: false,
