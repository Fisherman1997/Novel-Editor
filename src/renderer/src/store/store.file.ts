...
import { CurrentInfo, characterType, worldViewType } from '../../../types/types'
import { mainStore } from './store.main'

export type nameType = 'book' | 'volumeName' | 'worldName' | 'characterName' | 'chapterName'
export type deleteType = 'volume' | 'world' | 'character' | 'chapter'
export const fileStore = defineStore('fileInfo', {
    ...
    actions: {
         ...
         * @param index
         */
        delete(type: deleteType, index: [number, number?]) {
            mainStore().changeFile('isChangeFile', true)
            if (type === 'volume') {
