<template>
    <div class="app-right" :style="{ width: `${mainStore.rightPanelWidth}px` }">
        ...
        <div class="panel-content">
            ...
            <AgentPanel v-else-if="activeTab === 'assistant'" />
        </div>
...
type TabKey = 'characters' | 'worldViews' | 'annotations' | 'assistant'

const activeTab = computed({
    get: () => mainStore.rightPanelTab,
    set: (v: TabKey) => {
        mainStore.rightPanelTab = v
    }
})

const tabs = computed(() => [
    {
        key: 'characters' as TabKey,
        label: '人物',
        icon: 'users',
        count: novelStore.characters.length
    },
    {
        key: 'worldViews' as TabKey,
        label: '设定',
        icon: 'globe',
        count: novelStore.worldViews.length
    },
    {
        key: 'annotations' as TabKey,
        label: '标注',
        icon: 'message-square',
        count: novelStore.allAnnotations.length
    },
    {
        key: 'assistant' as TabKey,
        label: '助手',
        icon: 'bot',
        count: novelStore.agent.sessions.length
    }
])
