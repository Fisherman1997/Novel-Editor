  ...
  <aside class="app-left-nav">
    ...
    <div class="book-header">
      ...
      <div class="book-actions">
        ...
        <el-tooltip content="搜索章节" placement="bottom" :show-after="400">
          <button class="icon-btn" :class="{ 'is-active': filterText }" @click="focusFilter">
    ...
    <div class="chapter-list">
      <template v-if="visibleVolumes.length > 0">
        <div
          v-for="volume in visibleVolumes"
          :key="volume.index"
          class="volume"
        >
          <div
            ...
            :class="{
              ...
              'is-drag-over': volumeDrag.dragState.value.dragOverIndex === volume.index
            }"
          ...
          >
            <button class="icon-btn chevron" @click.stop="toggleVolume(volume.index)">
              <AppIcon name="chevron-right" :size="13" :class="{ 'is-expanded': isVolumeExpanded(volume.index) }" />
            </button>
            ...
            <div class="row-actions">
              ...
              <el-tooltip content="更多操作" placement="top" :show-after="400">
                <button class="icon-btn" @click.stop="showVolumeMenu($event, volume.index)">
          ...
          <div v-if="isVolumeExpanded(volume.index) || filterText" class="chapters">
            <div
              ...
              :class="{
                ...
                'is-drag-over': chapterDragOverIndex === `${volume.index}-${chapter.index}`
              }"
              :draggable="!filterText"
              @click="selectChapter(volume.index, chapter.index)"
              @contextmenu.prevent="showChapterMenu($event, volume.index, chapter.index)"
            ...
            >
              <span class="chapter-grip"><AppIcon name="grip-vertical" :size="12" /></span>

              <div class="chapter-name">
                <InlineEdit
                  :value="chapter.data.chapterName"
                  @change="(val) => novelStore.updateChapterName(volume.index, chapter.index, val)"
                />
              </div>
              ...
              <div class="row-actions">
                <el-tooltip :content="chapter.data.chapterSummary || '编辑章说明'" placement="top" :show-after="400">
                  <button
                    class="icon-btn"
                    :class="{ 'has-summary': !!chapter.data.chapterSummary }"
                    @click.stop="editChapterSummary(volume.index, chapter.index)"
                  >
                    <AppIcon name="info" :size="13" />
                  </button>
                </el-tooltip>
                <button class="icon-btn is-danger" @click.stop="deleteChapter(volume.index, chapter.index)" title="删除章节">
    ...
    >
      <el-input
        ...
        type="textarea"
        :rows="6"
        :placeholder="summaryDialogType === 'volume' ? '这一卷主要讲什么...' : '这一章主要讲什么...'"
      />
      <template #footer>
...
const visibleVolumes = computed(() => {
  ...
  return novelStore.volumes
    ...
    .filter((volume) => volume.data.volumeName.toLowerCase().includes(lowerFilter.value) || volume.chapters.length > 0)
})
...
const addChapter = (volumeIndex: number) => {
  const index = novelStore.insertChapterAt(volumeIndex, novelStore.volumes[volumeIndex]?.chapters.length || 0)
  expandedVolumes.value.add(volumeIndex)
  if (index >= 0) {
...
const editChapterSummary = (volumeIndex: number, chapterIndex: number) => {
  ...
  summaryDialogContent.value = novelStore.volumes[volumeIndex].chapters[chapterIndex].chapterSummary || ''
  showSummaryDialog.value = true
}
...
// ---------- 右键菜单 ----------
const { isVisible: menuVisible, x: menuX, y: menuY, items: menuItems, show: showMenu, hide: hideMenu } = useContextMenu()
...
const showVolumeMenu = (event: MouseEvent, volumeIndex: number) => {
  showMenu(event, [
    ...
    { divider: true },
    { label: '在上方插入卷', icon: 'arrow-up', action: () => novelStore.insertVolumeAt(volumeIndex) },
    {
      label: '在下方插入卷',
      icon: 'arrow-down',
      action: () => {
...
const showChapterMenu = (event: MouseEvent, volumeIndex: number, chapterIndex: number) => {
  showMenu(event, [
    ...
    { label: '编辑章说明', icon: 'info', action: () => editChapterSummary(volumeIndex, chapterIndex) },
    { divider: true },
    {
      label: '在上方插入章节',
      icon: 'arrow-up',
    ...
    { divider: true },
    { label: '删除章节', icon: 'trash', danger: true, action: () => deleteChapter(volumeIndex, chapterIndex) }
  ])
}
