    ...
    <div class="search-dialog">
      <div class="search-input-row">
        ...
        <div class="result-nav">
          ...
          <span class="result-count muted" v-else-if="query">无结果</span>
          <button class="icon-btn" :disabled="resultCount === 0" title="上一个 (Enter 上一个)" @click="prevResult">
            <AppIcon name="arrow-up" :size="14" />
          </button>
          <button class="icon-btn" :disabled="resultCount === 0" title="下一个 (Enter)" @click="nextResult">
      ...
      <div v-if="showReplace" class="replace-input-row">
        ...
        <el-button :disabled="resultCount === 0" type="warning" plain @click="replaceCurrentAll">
          全部替换
        </el-button>
      </div>

      <div class="search-options">
        <el-check-tag :checked="options.caseSensitive" @change="toggleOption('caseSensitive')">区分大小写</el-check-tag>
        <el-check-tag :checked="options.wholeWord" @change="toggleOption('wholeWord')">全词匹配</el-check-tag>
        <el-check-tag :checked="options.useRegex" @change="toggleOption('useRegex')">正则表达式</el-check-tag>
...
const {
  query, replaceText, options, results, currentIndex,
  isSearching, resultCount,
  search, next, prev, goToResult: goToSearchResult, highlightText
} = useSearch()

const visible = ref(false)
const showReplace = ref(false)

watch(() => editorStore.search.isOpen, (isOpen) => {
...
// 跳转搜索结果：切换章节 + 请求滚动定位
const scrollToResult = (result: { volumeIndex: number; chapterIndex: number; position: number; length: number }) => {
