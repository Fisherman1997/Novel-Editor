<template>
    ...
    >
        <div class="annotation-dialog">
            <!-- 选中文本预览（仅创建模式） -->
            <div v-if="dialogMode === 'create' && createSelection" class="selected-text">
                <label>选中文本</label>
                <div class="text-preview">{{ createSelection.text }}</div>
            </div>

            <!-- 编辑：当前标注文本 -->
            <div v-if="dialogMode === 'edit' && editingAnnotation" class="selected-text">
                <label>标注文本</label>
...
const canConfirm = computed(() => {
    ...
    if (annotationType.value === 'link') {
        return (
            annotationContent.value.trim().length > 0 ||
            !!selectedCharacterId.value ||
            selectedChapterIndex.value !== undefined
        )
    }
    return false
})
